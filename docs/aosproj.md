...
<!--
  安装一个 systemd .service 文件，并自动生成完整的 maintainer scripts：
    postinst (configure, 新装)  → systemctl enable + systemctl start
                                    (UsePreset=true 时改用 systemctl preset)
  ...
  UsePreset  = true 时 postinst 使用 systemctl preset 代替 systemctl enable。
               用于包同时提供了 systemd preset 文件（如
               /usr/lib/systemd/system-preset/90-*.preset）的场景，
               让 systemd 按 preset 策略决定是否启用，而非硬启用。
               默认为 false。
